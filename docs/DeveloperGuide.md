...
3. **Validate person session dates against group sessions on load**:
   Currently, a person's session records are not cross-checked against the group's session list during loading. A manually edited save file could contain a person with a session date that does not exist in the group's session list, and TAA will load it without warning. <br> Functionally, there is currently no problem as view/exportview will still show all sessions (union of group's and student's sessions). A future enhancement would validate person session dates against the group's session list during load, and skip or warn on mismatched entries.